...
{
    public record CompanyReportsQuery(
        string? NID,
        DateTime? From,
        DateTime? To,
        int pageIndex = 1,
    ...
    ) : IRequestBase<PagingViewModel<CompaniesReportsDTO>>;

    public class CompanyReportsQueryHandler : RequestHandlerBase<Company, CompanyReportsQuery, PagingViewModel<CompaniesReportsDTO>>
    {
        public CompanyReportsQueryHandler(RequestHandlerBaseParameters<Company> requestParameters) : base(requestParameters)
        {
        }

        public async override Task<RequestResult<PagingViewModel<CompaniesReportsDTO>>> Handle(CompanyReportsQuery request, CancellationToken cancellationToken)
        {
            ...
            //{
            //    predicate = predicate.And(c => c.ID == userId);
            ...
            return RequestResult<PagingViewModel<CompaniesReportsDTO>>.Success(query);
        }
