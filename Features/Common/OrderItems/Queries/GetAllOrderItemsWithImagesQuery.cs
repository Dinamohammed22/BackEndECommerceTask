...
{
    public record GetAllOrderItemsWithImagesQuery(string OrderId):IRequestBase<IEnumerable<OrderItemsDTO>>;
    public class GetAllOrderItemsWithImagesQueryHandler : RequestHandlerBase<OrderItem, GetAllOrderItemsWithImagesQuery, IEnumerable<OrderItemsDTO>>
    {
        public GetAllOrderItemsWithImagesQueryHandler(RequestHandlerBaseParameters<OrderItem> requestParameters) : base(requestParameters)
        {
        }
        public async override Task<RequestResult<IEnumerable<OrderItemsDTO>>> Handle(GetAllOrderItemsWithImagesQuery request, CancellationToken cancellationToken)
        {
            var orderItems = await _repository.Get(x => x.OrderId == request.OrderId)
                .Include(x => x.Product)
            ...
            foreach (var item in orderItems)
            {
                ...
                var orderItemDTO = item.MapOne<OrderItemsDTO>() with
                {
