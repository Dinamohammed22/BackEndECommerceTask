...
{
    public class OrdersReportsQuery : IRequestBase<PagingViewModel<OrderReportsDTO>>
    {
    ...
    public class OrdersReportsQueryHandler : RequestHandlerBase<Order, OrdersReportsQuery, PagingViewModel<OrderReportsDTO>>
    {
        public OrdersReportsQueryHandler(RequestHandlerBaseParameters<Order> requestParameters) : base(requestParameters)
        {
        }

        public async override Task<RequestResult<PagingViewModel<OrderReportsDTO>>> Handle(OrdersReportsQuery request, CancellationToken cancellationToken)
        {
            ...
            PagingViewModel<OrderReportsDTO> query =await _repository.Get(predicate)
                .Include(o => o.Client)
                .Map<OrderReportsDTO>().ToPagesAsync(request.PageIndex, request.PageSize); ;

            return RequestResult<PagingViewModel<OrderReportsDTO>>.Success(query);
        }
    }
}
