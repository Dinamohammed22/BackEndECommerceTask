...
{
    public class GetAllCompanyOrdersReportsQuery : IRequestBase<PagingViewModel<OrderReportsDTO>>
    {
    ...
    public class GetAllCompanyOrdersReportsQueryHandler : RequestHandlerBase<Order, GetAllCompanyOrdersReportsQuery, PagingViewModel<OrderReportsDTO>>
    {
        public GetAllCompanyOrdersReportsQueryHandler(RequestHandlerBaseParameters<Order> requestParameters) : base(requestParameters)
        {
        }

        public async override Task<RequestResult<PagingViewModel<OrderReportsDTO>>> Handle(GetAllCompanyOrdersReportsQuery request, CancellationToken cancellationToken)
        {
            ...
            var query = _repository.Get(predicate)
                ...
                .Select(order => new OrderReportsDTO
                {
                    ID = order.ID,
                ...
                .Where(oi => oi.Product.CompanyId == companyId)
                .Sum(oi => oi.NetPrice * oi.Quantity),
            ...
            return RequestResult<PagingViewModel<OrderReportsDTO>>.Success(pageing);
        }
    }
}
