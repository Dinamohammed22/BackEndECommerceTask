...
{
    public record ExportProductsReportQuery(string? ProductName, DateTime? From,DateTime? To) : IRequestBase<ExportProductsReportDTO>;
    public class ExportProductsReportQueryHandler : RequestHandlerBase<Product, ExportProductsReportQuery, ExportProductsReportDTO>
    {
        public ExportProductsReportQueryHandler(RequestHandlerBaseParameters<Product> requestParameters) : base(requestParameters)
        {
        }

        public async override Task<RequestResult<ExportProductsReportDTO>> Handle(ExportProductsReportQuery request, CancellationToken cancellationToken)
        {
            var CompanyID = "";
            ...
            var products = productEntities.Select(p => new ProductsReportDTO
            {
                ID = p.ID,
                ProductName = p.Name,
            ...
            using (var package = new ExcelPackage())
            {
                ...
                var headers = new[]
                 {
                   "Product Name", "Company Name", "Category Name", "Subcategory Name", "Total Price", "Total Weight"
                 };
                ...
                for (var i = 0; i < products.Count; i++)
                {
                    ...
                    worksheet.Cells[i + 2, 5].Value = product.TotalPrice ?? 0;
                    worksheet.Cells[i + 2, 6].Value = product.TotalWeight ?? 0;
                ...
                var exportProductsDTO = new ExportProductsReportDTO(fileContents, fileName, contentType);

                return RequestResult<ExportProductsReportDTO>.Success(exportProductsDTO);
            }
        }
    }
}
